...
dom               = parse("/home/pi/catkin_ws/src/eyes/scripts/graphics/eye.svg")
vb                = get_view_box(dom)
...
lidMap    = pi3d.Texture("graphics/green.png"   , mipmap=False,
              filter=pi3d.constants.GL_LINEAR, blend=True)
...
def frame(p):
	global startX, startY, destX, destY, curX, curY, X, Y
...
while True:
	...
	with open('/tmp/conf.txt', 'r') as file:
		while True:
			...
			if not line:
				break
			eyesConfig.append(line.strip())
	size = float(eyesConfig[0])
	randomPupils = eval(eyesConfig[1])
	randomBehavior = eval(eyesConfig[2])
	X = float(eyesConfig[3])
	Y = float(eyesConfig[4])
	AUTOBLINK = eval(eyesConfig[5])
	CRAZY_EYES = eval(eyesConfig[6])
	if randomPupils:
		v = random.random()
